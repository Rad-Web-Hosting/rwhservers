...
class HookIntegrator
{
    ...
    protected function addIntegration($className = null)
    {
        ...
        $integrationClassName = '\ModulesGarden\ProductsReseller\Server\rwhservers\App\Integrations\\' . ($this->isAdmin ? 'Admin' : 'Client') . '\\' . $className;
        if (!class_exists($integrationClassName) || !is_subclass_of($integrationClassName, \ModulesGarden\ProductsReseller\Server\rwhservers\Core\Hook\AbstractHookIntegrationController::class))
        {
            return false;
        }
    ...
    {
        if (!is_subclass_of($instance, \ModulesGarden\ProductsReseller\Server\rwhservers\Core\Hook\AbstractHookIntegrationController::class))
        {
            return false;
        }
    ...
    public function loadIntegrationData()
    {
        foreach ($this->integrations as $integration)
        {
            ...
            /** @var
             * $integrationResult \ModulesGarden\ProductsReseller\Server\rwhservers\Core\UI\View
             */
            $integrationResult = call_user_func([di($callbackData[0]), $callbackData[1]]);
            if (!($integrationResult instanceof \ModulesGarden\ProductsReseller\Server\rwhservers\Core\UI\View))
            {
    ...
    public function isIntegrationApplicable($integration = null)
    {
        ...
        if ((!is_subclass_of($integrationCallback[0], \ModulesGarden\ProductsReseller\Server\rwhservers\Core\Http\AbstractController::class)
             && !is_subclass_of($integrationCallback[0], \ModulesGarden\ProductsReseller\Server\rwhservers\Core\Http\AbstractClientController::class))
