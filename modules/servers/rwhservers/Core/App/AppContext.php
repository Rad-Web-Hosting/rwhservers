...
use ModulesGarden\ProductsReseller\Server\rwhservers\Core\HandlerError\WhmcsErrorManagerWrapper;
...
class AppContext
{
    ...
    public function __construct()
    {
        ...
        if ($this->debugMode)
        {
            spl_autoload_register(['\ModulesGarden\ProductsReseller\Server\rwhservers\Core\App\AppContext', 'loadClassLoader'], true, false);
        }
    }
    ...
    {
        $rawClass = trim($class, '\\');
        $pos      = strpos($rawClass, 'ModulesGarden\ProductsReseller\Server\rwhservers');
        if ($pos === 0)
        {
