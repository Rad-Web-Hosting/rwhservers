...
use ModulesGarden\ProductsReseller\Server\rwhservers\Core\App\Controllers\ResponseResolver;
use ModulesGarden\ProductsReseller\Server\rwhservers\Core\App\Installer\ModuleInstaller;
...
use ModulesGarden\ProductsReseller\Server\rwhservers\Core\Models\Whmcs\Server;
 ...
 */
class ConfigOptions extends \ModulesGarden\ProductsReseller\Server\rwhservers\Core\App\Controllers\Instances\AddonController
{
    use AppParams;
    ...
    public function execute($params = null)
    {
        ...
        if (($this->getRequestValue('action') === 'module-settings' || ($this->getRequestValue('loadData') && $this->getRequestValue('ajax') == '1')))
        {
            ...
            try
            {
                $this->setAppParam('IntegrationControlerName', \ModulesGarden\ProductsReseller\Server\rwhservers\App\Http\Actions\ConfigOptions::class);
                $this->setAppParam('IntegrationControlerMethod', 'runExecuteProcess');

                $configOptionsController = new \ModulesGarden\ProductsReseller\Server\rwhservers\App\Http\Actions\ConfigOptions();
        ...
        else
        {
            if ($this->getRequestValue('action') === 'save')
            {
                ...
                return [\ModulesGarden\ProductsReseller\Server\rwhservers\App\Http\Actions\ConfigOptions::class, 'runExecuteProcess'];
            }
        }
    ...
    public function isCorrectServerType()
    {
        try
        {
            if (class_exists('\ModulesGarden\ProductsReseller\Server\rwhservers\App\Http\Actions\MetaData'))
            {
                $metaDataController = new \ModulesGarden\ProductsReseller\Server\rwhservers\App\Http\Actions\MetaData();
    ...
    public function addRequiredCustomFields()
    {
        ...
        $product = new \ModulesGarden\ProductsReseller\Server\rwhservers\Packages\WhmcsService\Product($pid);
