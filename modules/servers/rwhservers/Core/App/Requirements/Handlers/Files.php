...
namespace ModulesGarden\ProductsReseller\Server\rwhservers\Core\App\Requirements\Handlers;

use ModulesGarden\ProductsReseller\Server\rwhservers\Core\App\Requirements\Handler;
use ModulesGarden\ProductsReseller\Server\rwhservers\Core\App\Requirements\HandlerInterface;
use ModulesGarden\ProductsReseller\Server\rwhservers\Core\App\Requirements\Instances\Files as FilesInstance;
...
class Files extends Handler implements HandlerInterface
{
    ...
    public function getFullPath($recordPath = null)
    {
        ...
        {
            return str_replace(FilesInstance::WHMCS_PATH, \ModulesGarden\ProductsReseller\Server\rwhservers\Core\ModuleConstants::getFullPathWhmcs(),
                str_replace('/', DIRECTORY_SEPARATOR, $recordPath));
        }

        if (stripos($recordPath, FilesInstance::MODULE_PATH) === 0)
        {
            return str_replace(FilesInstance::MODULE_PATH, \ModulesGarden\ProductsReseller\Server\rwhservers\Core\ModuleConstants::getModuleRootDir(),
                str_replace('/', DIRECTORY_SEPARATOR, $recordPath));
        }
    ...
    {
        $fileValidator = new \ModulesGarden\ProductsReseller\Server\rwhservers\Core\FileReader\PathValidator();
