...
trait Template
{
    ...
    {
        return ModuleConstants::getTemplateDir() . DIRECTORY_SEPARATOR . $this->templateScope . DIRECTORY_SEPARATOR . $this->templateMainDir . DIRECTORY_SEPARATOR;
    }

    public function changeTemplateSet($templateSet)
    {
        if (file_exists(ModuleConstants::getTemplateDir() . DIRECTORY_SEPARATOR . $this->templateScope . DIRECTORY_SEPARATOR . $this->templateMainDir . DIRECTORY_SEPARATOR . $templateSet))
        {
    ...
    protected function loadTemplateDir()
    {
        ...
        $base  = $isCore = null;
        if (str_contains($class, 'ModulesGarden\ProductsReseller\Server\rwhservers\Core\UI\\'))
        {
            $base   = str_replace('ModulesGarden\ProductsReseller\Server\rwhservers\Core\UI\\', '', trim($class, '\\'));
            $isCore = true;
        }

        if (str_contains($class, 'ModulesGarden\ProductsReseller\Server\rwhservers\App\UI\\'))
        {
            $base = str_replace('ModulesGarden\ProductsReseller\Server\rwhservers\App\UI\\', '', trim($class, '\\'));
        }

        if (!$base)
        {
            $this->templateDir = $this->getDefaultTemplateDir() . $this->templateSet . DIRECTORY_SEPARATOR;
    ...
    {
        $basePath = implode(DIRECTORY_SEPARATOR, array_map('lcfirst', $pathParts));

        $this->templateDir = $this->getDefaultTemplateDir() . 'core' . DIRECTORY_SEPARATOR . $this->templateSet . ($basePath ? DIRECTORY_SEPARATOR . $basePath : '') . DIRECTORY_SEPARATOR;
    }
    ...
    protected function getBaseAppPatch($pathParts)
    {
        ...
        $controlerDir = ModuleConstants::getModuleRootDir() . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'UI' . DIRECTORY_SEPARATOR . $controler . DIRECTORY_SEPARATOR . 'Templates';

        $basePath = implode(DIRECTORY_SEPARATOR, array_map('lcfirst', $pathParts));

        $this->templateDir = $controlerDir . ($basePath ? DIRECTORY_SEPARATOR . $basePath : '') . DIRECTORY_SEPARATOR;
    }
    ...
    protected function evaluateTemplatePath()
    {
        if (!file_exists($this->templateDir . $this->templateName . '.tpl'))
        {
            ...
            if ($parent)
            {
                $new = \ModulesGarden\ProductsReseller\Server\rwhservers\Core\Helper\di($parent, null, true);
                if ($new)
                {
