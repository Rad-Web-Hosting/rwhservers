...
class ClientAreaSidebar
{
    ...
    {
        if (!$this->hosting || !$this->request->getSessionData('uid') || $this->hosting->domainstatus !== 'Active' || $this->hosting->product->servertype !== $this->moduleName)
        {
            return;
        }
...
//            'uri' => BuildUrl::getUrl('OneClickLogin', 'redirect',array_merge($params,['where' => $childName])),
    ...
    {
        if (!$this->hosting || !$this->request->getSessionData('uid') || $this->hosting->domainstatus !== 'Active')
        {
            return;
        }
